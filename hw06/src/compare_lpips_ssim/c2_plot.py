...
df = pd.read_csv('D:/graduate/113.2/CT/ct2024s/hw06/src/compare_lpips_ssim/chen/result.csv')

# 篩選出 SSIM 值在 0.3 到 0.7 之間的數據
df_filtered = df[(df['SSIM'] >= 0.56) & (df['SSIM'] <= 0.9) & 
                 (df['LPIPS'] >= 0.3) & (df['LPIPS'] <= 0.6)]
...
plt.xticks(ticks=np.arange(0.72, 0.86, 0.02)) ##SSIM
plt.yticks(ticks=np.arange(0.31, 0.56, 0.02))  ##LPIPS
...
# 儲存圖片
plt.savefig('chen.png', bbox_inches='tight')
